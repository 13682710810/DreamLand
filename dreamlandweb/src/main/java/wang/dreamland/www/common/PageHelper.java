...
public class PageHelper implements Interceptor {
	...
	public static class Page<E> implements Serializable {
		...
		* */
		...
		public int getPages() {
			// 计算总页数
			long totalPage = this.getTotal() / this.getPageSize() + ((this.getTotal() % this.getPageSize() == 0) ? 0 : 1);
		...
		public void setTotal(long total) {
			//计算总页码数：主要考虑每页记录数 pageSize 为1的情况，所以减去1，然后除以每页记录数取整数。
